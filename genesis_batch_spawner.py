...
"""
    ...
    def deploy_agent(cls, target: dict, agent_data: dict):
        ...
        <nav class="border-b border-slate-800 bg-slate-900/80 p-4 mb-6 flex justify-between items-center text-xs font-sans">
            ...
            <a href="{cls.RAPIDAPI_URL}" target="_blank" class="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded">
                ⚡ RapidAPI Pro Tier ($9.99/mo)
            </a>
        </nav>
        """
        footer = f"""
    ...
    def append_to_hub(target: dict, agent_data: dict):
        ...
        <div class="card bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-indigo-500/50 transition flex flex-col justify-between">
            ...
            <div class="flex items-center space-x-2 mt-2">
                ...
                <a href="{MassSwarmFactory.RAPIDAPI_URL}" target="_blank" class="bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold px-3 py-2 rounded transition">
                    RapidAPI ↗
                </a>
            </div>
        </div>"""
    ...
    def run_mass_spawn(cls):
        ...
        msg = (
            ...
            f"💳 *Monetization:* RapidAPI link + UPI active on all."
        )
        ...
        payload = json.dumps({
            ...
            "reply_markup": {
                "inline_keyboard": [
                    ...
                    [{"text": "⚡ RapidAPI Catalog", "url": cls.RAPIDAPI_URL}]
                ]
            }
        }).encode("utf-8")
        try:
